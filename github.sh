...
case "$1" in
  ...
  pre|rel)
    ...
    ZIP_FILE="${PROJECT_DIR}/cesium-plus-pod-assembly/target/${ZIP_BASENAME}"
    if [[ -f "${ZIP_FILE}" ]]; then
      result=$(curl -s -H ''"$GITHUT_AUTH"'' -H 'Content-Type: application/zip' -T "${ZIP_FILE}" "$upload_url?name=${ZIP_BASENAME}")
      browser_download_url=$(echo "$result" | grep -P "\"browser_download_url\":[ ]?\"[^\"]+" | grep -oP "\"browser_download_url\":[ ]?\"[^\"]+"  | grep -oP "https://[A-Za-z0-9/.-]+")
      ZIP_SHA256=$(cd ${PROJECT_DIR}/cesium-plus-pod-assembly/target && sha256sum "${ZIP_BASENAME}")
    ...
    SHA_FILE=${PROJECT_DIR}/cesium-plus-pod-assembly/target/${SHA_BASENAME}
